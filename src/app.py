...
class PDFTemplateApp:
    ...
    def _create_menu(self) -> None:
        ...
        file_menu.add_command(
            label="Open PDF...", command=self._open_pdf, accelerator="Ctrl+O"
        )
        file_menu.add_separator()
        file_menu.add_command(label="Load Template...", command=self._load_template)
        file_menu.add_command(
            label="Save Template...", command=self._save_template, accelerator="Ctrl+S"
        )
        file_menu.add_separator()
        ...
        csv_menu.add_command(
            label="Configure Mapping...", command=self._configure_mapping
        )
        ...
        gen_menu.add_command(
            label="Generate PDFs...", command=self._generate_pdfs, accelerator="Ctrl+G"
        )
    ...
    def _create_sidebar(self, parent: tk.Widget) -> None:
        ...
        tk.Button(
            actions_frame,
            text="Clear All",
            command=self._clear_placeholders,
            bg="#5a3030",
            fg="white",
            width=10,
        ...
        self.csv_label = tk.Label(
            sidebar, text="No CSV loaded", bg="#2b2b2b", fg="#888888", font=("Arial", 9)
        )
        self.csv_label.pack(fill=tk.X, pady=5)

        # Mapping status label
        self.mapping_label = tk.Label(
            sidebar, text="", bg="#2b2b2b", fg="#4a9eff", font=("Arial", 9)
        )
        ...
        tk.Label(
            range_inputs, text="Start:", bg="#2b2b2b", fg="white", font=("Arial", 9)
        ).pack(side=tk.LEFT)
        ...
        tk.Label(
            range_inputs, text="End:", bg="#2b2b2b", fg="white", font=("Arial", 9)
        ).pack(side=tk.LEFT)
        ...
        tk.Label(
            range_frame,
            text="Leave End empty for all rows",
            bg="#2b2b2b",
            fg="#666666",
            font=("Arial", 8),
        ).pack(anchor=tk.W)
    ...
    def _on_placeholder_moved(
        self, placeholder: Placeholder, new_x: float, new_y: float
    ) -> None:
    ...
    def _configure_mapping(self) -> None:
        ...
        if not placeholder_names:
            messagebox.showwarning(
                "No Placeholders", "Please add placeholders to the PDF first."
            )
            return

        dialog = CSVMappingDialog(
            self.root, self.csv_handler.headers, placeholder_names
        )
        result = dialog.show()
    ...
    def _generate_pdfs(self) -> None:
        ...
        if self.csv_handler.mapping is None or (
            not self.csv_handler.mapping and self.csv_handler.file_path
        ):
        ...
        if end_row_str:
            try:
                end_row = int(
                    end_row_str
                )  # Keep as 1-indexed for user, convert in generator
            except ValueError:
                end_row = None
        else:
            end_row = None
        ...
        progress_bar = ttk.Progressbar(
            progress_window, variable=progress_var, maximum=count
        )
        progress_bar.pack(fill=tk.X, padx=20, pady=10)

        progress_label = tk.Label(
            progress_window, text="0 / 0", bg="#2b2b2b", fg="#888888"
        )
        progress_label.pack()
        ...
        try:
            ...
            # If merging, use a temp directory for individual files
            ...
            # Merge if requested
            if merge_mode and len(generated) >= 1:
                ...
                messagebox.showinfo(
                    "Complete", f"Merged {len(generated)} pages into:\n{merged_path}"
                )
                self._set_status(f"Created merged PDF with {len(generated)} documents")
            else:
                progress_window.destroy()
...
"""
    ...
    def _show_about(self) -> None:
        ...
        messagebox.showinfo(
            ...
            "Built by Rishav Raj, https://www.github.com/rishavmngo",
        )
